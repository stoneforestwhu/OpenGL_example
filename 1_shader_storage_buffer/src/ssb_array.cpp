...
void compute() {
  ...
  glDeleteShader(cs_0);
  std::shared_ptr<char> cs_string = readShaderFile("shader/ssb_array.txt");
  ...
  glBufferData(GL_SHADER_STORAGE_BUFFER, 800, buf0_ptr, GL_DYNAMIC_COPY);
  ...
  glBufferData(GL_SHADER_STORAGE_BUFFER, 800, NULL, GL_DYNAMIC_COPY);
  ...
  GLint *readBuffer = (GLint *)calloc(4, 200); //  129 is enough
  if (!readBuffer) {
    assert(0 && "not enough space!");
  }
  GLuint error_code = glGetError();
  glGetBufferSubData(GL_SHADER_STORAGE_BUFFER, 0, sizeof(GLuint) * 200,
                     readBuffer);

  printData<int>((int*)readBuffer, 50, 4);
