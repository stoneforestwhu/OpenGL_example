...
void compute() {
  ...
  glDeleteShader(cs_0);
  std::shared_ptr<char> cs_string = readShaderFile("shader/ssb_align.txt");
  ...
  GLint *readBuffer = (GLint *)calloc(4, 192); //  129 is enough
  if (!readBuffer) {
    assert(0 && "not enough space!");
  }
  GLuint error_code = glGetError();
  glGetBufferSubData(GL_SHADER_STORAGE_BUFFER, 0, sizeof(GLuint) * 192,
                     readBuffer);

  printData<int>((int*)readBuffer, 48, 4);
