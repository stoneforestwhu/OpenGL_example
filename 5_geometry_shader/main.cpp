...
void draw(){
	...
	std::shared_ptr<char> vs_0_src_0_shared_ptr = readShaderFile("shader/gs.vert");
	...
	std::shared_ptr<char> ps_0_src_0_shared_ptr = readShaderFile("shader/gs.frag");
  ...
  glProg_0 = glCreateProgram();
	...
	//bufD_0=byte[80](file=bufD_0.bin)
	std::shared_ptr<char> shared_ptr_bufD_0 = readBufferFile("data/bufD_0.bin", 80);
	GLvoid* bufD_0 = shared_ptr_bufD_0.get();
