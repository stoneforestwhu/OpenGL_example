...
void compute() {
  ...
  glDeleteShader(cs_0);
  std::shared_ptr<char> cs_string = readShaderFile("shader/controlflow.txt");
  ...
  int buf0_ptr[8] = {0, 2, 1, 2, 1, 0, 1, 1};
  glBufferData(GL_SHADER_STORAGE_BUFFER, 32, buf0_ptr, GL_DYNAMIC_COPY);
  ...
  shared_ptr<int> resultData(new int[4]);
  glGetBufferSubData(GL_SHADER_STORAGE_BUFFER, 0, sizeof(GLuint) * 4, resultData.get());
  error_code = glGetError();
